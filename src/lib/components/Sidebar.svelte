...
<aside class="space-y-6">
	...
	<div class="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-raised)] p-4">
		<div class="grid grid-cols-2 gap-4">
			<div>
				...
				<div class="font-mono text-[15px] font-semibold text-[var(--color-text)]">{formatNumber(extension.download_count)}</div>
			</div>

			{#if github && !isMonorepo}
				<div>
					<div class="text-[12px] text-[var(--color-text-tertiary)]">Stars</div>
					<div class="font-mono text-[15px] font-semibold text-[var(--color-text)]">{formatNumber(github.stargazers_count)}</div>
				</div>
				<div>
					<div class="text-[12px] text-[var(--color-text-tertiary)]">Forks</div>
					<div class="font-mono text-[15px] font-semibold text-[var(--color-text)]">{formatNumber(github.forks_count)}</div>
				</div>
				<div>
					<div class="text-[12px] text-[var(--color-text-tertiary)]">Issues</div>
					<div class="font-mono text-[15px] font-semibold text-[var(--color-text)]">{formatNumber(github.open_issues_count)}</div>
				</div>
			{/if}
		</div>
	</div>
	...
	<div class="space-y-3 text-[13px]">
		{#if github?.license}
			<div>
				...
				<span class="ml-2 rounded-md bg-[var(--color-surface)] px-2 py-0.5 font-mono text-[12px] text-[var(--color-text-secondary)]">{github.license.spdx_id}</span>
			</div>
		{/if}
		...
		{#if extension.wasm_api_version}
			<div>
				...
				<span class="ml-2 font-mono text-[12px] text-[var(--color-text-secondary)]">v{extension.wasm_api_version}</span>
			</div>
		{/if}
	</div>
	...
	<!-- Provides -->
	<div>
		<h3 class="mb-2 text-[13px] font-semibold uppercase tracking-wide text-[var(--color-text-tertiary)]">Provides</h3>
	...
	<!-- Authors -->
	<div>
		<h3 class="mb-2 text-[13px] font-semibold uppercase tracking-wide text-[var(--color-text-tertiary)]">Authors</h3>
		<div class="space-y-1">
