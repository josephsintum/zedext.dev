<script lang="ts">
	...
	const primaryProvides = $derived(
		hit.provides.filter((p) => !['grammars', 'slash-commands', 'indexed-docs-providers'].includes(p))
	);
...
<a
	...
	class="card-hover group flex min-w-0 flex-col overflow-hidden rounded-lg border border-[var(--color-border)] border-l-[3px] border-l-transparent bg-[var(--color-surface-raised)] p-4 {accentBorder}"
	style="animation-delay: {index * 30}ms"
>
	<div class="mb-1.5 flex items-start justify-between gap-2">
		<h3 class="min-w-0 truncate font-mono text-[14px] font-semibold text-[var(--color-text)] group-hover:text-[var(--color-accent-hover)]">
			{hit.name}
		</h3>
		<span class="shrink-0 rounded-md bg-[var(--color-surface)] px-1.5 py-0.5 font-mono text-[11px] text-[var(--color-text-tertiary)]">
			v{hit.version}
		</span>
	</div>

	{#if hit.description}
		<p class="mb-3 min-w-0 line-clamp-2 text-[13px] leading-relaxed text-[var(--color-text-secondary)]">
			{hit.description}
		</p>
	{:else}
		<div class="mb-3"></div>
	...
	<div class="mt-3 flex items-center gap-3 border-t border-[var(--color-border)] pt-3 text-[12px] text-[var(--color-text-tertiary)]">
		<!-- Downloads -->
