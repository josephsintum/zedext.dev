		...
		>
			{#each displayHits as hit (hit.objectID)}
				...
				<a
					...
					class="group flex flex-col rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-raised)] p-4 transition-colors duration-150 {getHoverBorder(hit)}"
				>
					<!-- Name + downloads -->
					<div class="mb-1.5 flex items-start justify-between gap-2">
						<h3 class="min-w-0 truncate font-mono text-[14px] font-semibold text-[var(--color-accent)] group-hover:text-[var(--color-accent-hover)]">
							{hit.name}
						</h3>
						<span class="flex shrink-0 items-center gap-1 text-[12px] tabular-nums text-[var(--color-text-tertiary)]">
					...
					<!-- Description -->
					{#if hit.description}
						<p class="mb-3 line-clamp-2 text-[13px] leading-relaxed text-[var(--color-text-secondary)]">
							{hit.description}
						</p>
					{:else}
						<div class="mb-3"></div>
					...
					<!-- Footer: stars + author -->
					<div class="mt-auto flex items-center gap-3 border-t border-[var(--color-border)] pt-3 text-[12px] text-[var(--color-text-tertiary)]">
